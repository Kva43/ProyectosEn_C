...
static int getAlfanumerica(char* pResultado, int longitud);
...
static int getString(char* cadena, int longitud)
{
	...
	if(cadena != NULL && longitud > 0)
	{
		...
		if(fgets(bufferString,sizeof(bufferString),stdin) != NULL)		//lee la línea de texto y lo almacena en "bufferString".
		{
			if(bufferString[strnlen(bufferString,sizeof(bufferString))-1] == '\n')	//verifica si el usuario presionó "Enter" después de ingresar la cadena.
			{
				bufferString[strnlen(bufferString,sizeof(bufferString))-1] = '\0'; //Si el usuario presionó "Enter", ese carácter se remplaza por uno nulo.
			}
...
static int getInt(int* pResultado)
{
    ...
    if(	pResultado != NULL &&
    	...
    	esNumerica(bufferString, sizeof(bufferString) == 0))
	{
		retorno = 0;
...
int utn_getAlfanumerica(char* pResultado, int longitud,char* mensaje, char* mensajeError, int reintentos)
{
	...
	while(reintentos>=0)
	{
		reintentos--;
		printf("%s",mensaje);
		if(getAlfanumerica(bufferString,sizeof(bufferString)) == 0 && strnlen(bufferString,sizeof(bufferString)) < longitud )
		{
			strncpy(pResultado,bufferString,longitud);
			retorno = 0;
			break;
